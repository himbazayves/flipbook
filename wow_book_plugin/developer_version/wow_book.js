...
wowBook.prototype = {
	...
	isPageToKeep : function(page){
		if ( !page.keep ) var content = page.find('.wowbook-page-content');
	...
	setCovers : function(){
		...
		for(var i=0,l=this.pages.length-1;i<=l;i++){
			...
			if ( page.insideCover ) {
				if ( page.onLeft ) { book.insideCoverLeft = page }
				else { book.insideCoverRight = page }
				if ( book.differentPageSizes ) page.keep = true;
			}
		}
	}, // setCovers
	...
	showPage : function(pageIndex, updateLocationHash) {
		...
		if ( this.differentPageSizes ){
			if ( leftBelow!==null )  this.loadPage( this.insideCoverLeft );
			if ( rightBelow!==null ) this.loadPage( this.insideCoverRight );
		}
	...
	*/
	...
	zoom : function(level, duration, options){
		...
		if (x==undefined) {
			x = this.pageWidth*( this._singlePage ? 1 : 2 ) * this.currentScale * this.opts.zoomFocusX;
		}

		if (y==undefined) {
			y = this.pageHeight * this.currentScale * this.opts.zoomFocusY;
		}
	...
	initThumbnails: function(){
		...
		this.thumbnailsContainer = $("<div class='wowbook-thumbnails'>").append(
			...
			"<a class='wowbook-back wowbook-button wowbook-control wowbook-control-back'></a>"+
			"<div class='wowbook-clipper'><ul></ul></div>"+
			"<a class='wowbook-next wowbook-button wowbook-control wowbook-control-next'></a>"+
			"</div>")
			.css('display', 'none')
		...
		this.thumbnailsBackButton = container.find('.wowbook-back.wowbook-button').on("click",function(){
			book._moveThumbnailsList( book.thumbnailsClipper[dimension]() );
		})
		this.thumbnailsNextButton = container.find('.wowbook-next.wowbook-button').on("click",function(){
			book._moveThumbnailsList( -book.thumbnailsClipper[dimension]() );
		})
	...
	scrollBarWidth : function(){
		var outerDiv = $("<div style='width:50px;height:60px;overflow-y:auto;display:inline-block;position: absolute;left:-1000px;'></div>").appendTo("body");
		var outerWidth = outerDiv.width();
		var innerDiv = $("<div style='width:100%;height:100px;display:inline-block;'>Hi!<br/>He<br/>llo!</div>").appendTo(outerDiv)
	...
	createThumbnail : function(pageIndex, config){
		...
		if ( this.opts.thumbnailNumber || this.opts.thumbnailsNumber ) {
			var numberString = this.opts.onThumbnailNumberString ? this.opts.onThumbnailNumberString( this, page, pageIndex ) : pageIndex+1;
			thumb.find(".wowbook-overlay").append('<div class="wowbook-thumbnail-number">'+( numberString )+'</div>');
		}
		...
		thumb.on("click",function(){
			...
			if ( book.opts.thumbnailsHideOnClick ) { book.hideThumbnails(); }
		});
	...
	setPDF: function( url ){
		...
		PDFJS.getDocument( url, null, null, this.opts.onPDFLoadProgress ).then(function( pdfDoc ){
			book.pdfDoc = pdfDoc;
			...
			$(book.pages[0] || book.elem).html( "Error: "+error.message );
			if ( location.protocol=="file:" ) {
				if ( !this.pdfLocalErrorMsg ) this.pdfLocalErrorMsg =
				...
				$(book.pages[0] || book.elem).append( this.pdfLocalErrorMsg );
			}
	...
	*/
	pdfRenderPage: function( pageIndex, canvas, callback, scale, type ) {
		...
		this.pdfDoc.getPage( pdfIndex ).then(function(page){
			...
			if ( book.opts.pdfUpdateRender && type!="thumb" ) {
				...
				if ( book.opts.pdfRoundWidth && (( desiredWidth%1 ) >= 0.5) ) desiredWidth = Math.round( desiredWidth );
	...
	generateCSSClass: function( styles, className ){
		...
		generateCSSRule( "."+className+" .wowbook-control", styles );
		generateCSSRule( "."+className+" .wowbook-control.remove-sticky-hover-style:hover", styles );
	...
	controllify : function(controls){
		...
		var url = opts.downloadURL || opts.downloadurl || opts.downloadUrl || ( $.isPlainObject( opts.pdf ) ? opts.pdf.url : opts.pdf  );
		if ( url ) $(controls.download).attr("href", url).attr("download", url);

		var homeURL = opts.homeURL || opts.homeurl || opts.homeUrl;
		...
		var controlList = ("zoomIn zoomOut zoomReset left lastLeft right lastRight next back first last slideShow flipSound "
		                  +"thumbnails fullscreen toc currentPage download find home").split(" "),
		    controlName;
	...
	setStrings: function( strings ){
		...
		setControlTitleAttribute( "homeTooltip", "home" );
	...
	createToolbar : function(controls, parent){
		...
		controls = controls.trim().split(/\s*,\s*/);
		var controlsObj = {};
		var control;
